...
type consensusProcess struct {
	...
	certficiationTracker          *certficiationTracker
	cfg                           config.Config
	...
	certificationEligibilityCount uint16
}
...
func newConsensusProcess(cfg config.Config, instanceID instanceID, s *Set, oracle Rolacle, stateQuerier StateQuerier,
	...
	proc := &consensusProcess{
		...
		certficiationTracker: newCertificationTracker(cfg.N),
		cfg:                  cfg,
...
func (proc *consensusProcess) isHareCertifier(ctx context.Context) bool {
	logger := proc.WithContext(ctx)
	...
	proof, err := proc.oracle.Proof(ctx, types.LayerID(proc.instanceID), hareEligibility)
	if err != nil {
	...
	proc.certificationEligibilityCount = eligibilityCount
	if err != nil {
		logger.With().Error("failed to check eligibility", log.Err(err))
		return false
	}
	return eligibilityCount > 0 //true if eligible, false if not
}
