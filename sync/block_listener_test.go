...
type mockP2P struct {
	f func(name string) chan service.GossipMessage
}

func (m *mockP2P) RegisterGossipProtocol(name string) chan service.GossipMessage {
	return m.f(name)
}

func (m *mockP2P) RegisterDirectProtocolWithChannel(protocol string, ingressChannel chan service.DirectMessage) chan service.DirectMessage {
	return make(chan service.DirectMessage, 100)
}

func (m *mockP2P) SendWrappedMessage(nodeID p2pcrypto.PublicKey, protocol string, payload *service.DataMsgWrapper) error {
	return nil
}

func (m *mockP2P) SubscribePeerEvents() (chan p2pcrypto.PublicKey, chan p2pcrypto.PublicKey) {
	return make(chan p2pcrypto.PublicKey),  make(chan p2pcrypto.PublicKey)
}

type mockMesh struct {
	GetBlockFunc func (id types.BlockID) (*types.Block, error)
	AddBlockWithTxsFunc func(blk *types.Block, txs []*types.AddressableSignedTransaction, atxs []*types.ActivationTx) error
}

func (m *mockMesh) GetBlock(id types.BlockID) (*types.Block, error) {
	return m.GetBlockFunc(id)
}

func (m *mockMesh) AddBlockWithTxs(blk *types.Block, txs []*types.AddressableSignedTransaction, atxs []*types.ActivationTx) error {
	return m.AddBlockWithTxsFunc(blk, txs, atxs)
}

type mockSync struct {
	IsSyncedFunc func() bool
	BlockSyntacticValidationFunc func(block *types.Block) ([]*types.AddressableSignedTransaction, []*types.ActivationTx, error)
}

func (m *mockSync) BlockSyntacticValidation(block *types.Block) ([]*types.AddressableSignedTransaction, []*types.ActivationTx, error) {
	return m.BlockSyntacticValidationFunc(block)
}

func (m *mockSync) IsSynced() bool {
...
func TestBlockListener_ListenToGossipBlocks(t *testing.T) {
	...
	mych := make(chan service.GossipMessage)
	p2 := &mockP2P{f: func(name string) chan service.GossipMessage {
		return mych
	}}
...
func (msg *p2pMsg) Bytes() []byte {
	return msg.data
}

func (msg *p2pMsg) Sender() p2pcrypto.PublicKey {
	return p2pcrypto.NewRandomPubkey()
}

func (msg *p2pMsg) ReportValidation(protocol string) {
