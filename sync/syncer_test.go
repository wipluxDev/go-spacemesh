...
//
func atx(lyr types.LayerID, key string, view []types.BlockID) *types.ActivationTx {
	coinbase := address.HexToAddress("aaaa")
	poetRef := []byte{0xba, 0xbe}
	return types.NewActivationTx(types.NodeId{Key: key, VRFPublicKey: []byte(RandStringRunes(5))}, coinbase, 0, *types.EmptyAtxId, lyr, 1, *types.EmptyAtxId, 0, view, nipst.NewNIPSTWithChallenge(&common.Hash{}, poetRef))
}
//
