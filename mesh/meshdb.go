...
func (m *MeshDB) GetBlock(id types.BlockID) (*types.Block, error) {
	...
	if blkh := m.blockCache.Get(id); blkh != nil {
		m.Log.Debug("running GetBlock (%v) took %v read from cache: %v",id, time.Since(t), true)
		return blkh, nil
	}
	...
	m.Log.Debug("running GetBlock (%v) took %v read from cache: %v",id, time.Since(t), false)
	return mbk, err
}
