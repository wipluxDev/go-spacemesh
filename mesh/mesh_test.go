...
func addLayer(r *require.Assertions, id types.LayerID, layerSize int, msh *Mesh) *types.Layer {
	for i := 0; i < layerSize; i++ {
		txIDs, _ := addManyTXsToPool(r, msh, id, 4)
...
func TestMesh_AddLayerGetLayer(t *testing.T) {
	...
	txIDs1, _ := addManyTXsToPool(r, msh, id, 4)
	txIDs2, _ := addManyTXsToPool(r, msh, id, 3)
	txIDs3, _ := addManyTXsToPool(r, msh, id, 6)
...
func TestMesh_LatestKnownLayer(t *testing.T) {
	msh := getMesh("t6")
	defer msh.Close()
	msh.SetLatestLayer(3)
	msh.SetLatestLayer(7)
	msh.SetLatestLayer(10)
	msh.SetLatestLayer(1)
	msh.SetLatestLayer(2)
	assert.True(t, msh.LatestLayer() == 10, "wrong layer")
}
...
func TestMesh_WakeUp(t *testing.T) {
	...
	r := require.New(t)
	txIDs1, _ := addManyTXsToPool(r, msh, 1, 4)
	txIDs2, _ := addManyTXsToPool(r, msh, 2, 3)
	block1 := types.NewExistingBlock(1, []byte("data1"), txIDs1)
	block2 := types.NewExistingBlock(2, []byte("data2"), txIDs2)
...
func TestMesh_OrphanBlocks(t *testing.T) {
	...
	r := require.New(t)
	txIDs1, _ := addManyTXsToPool(r, msh, 1, 4)
	txIDs2, _ := addManyTXsToPool(r, msh, 1, 3)
	txIDs3, _ := addManyTXsToPool(r, msh, 2, 6)
	txIDs4, _ := addManyTXsToPool(r, msh, 2, 7)
	txIDs5, _ := addManyTXsToPool(r, msh, 3, 3)
	block1 := types.NewExistingBlock(1, []byte("data data data1"), txIDs1)
	block2 := types.NewExistingBlock(1, []byte("data data data2"), txIDs2)
	block3 := types.NewExistingBlock(2, []byte("data data data3"), txIDs3)
	block4 := types.NewExistingBlock(2, []byte("data data data4"), txIDs4)
	block5 := types.NewExistingBlock(3, []byte("data data data5"), txIDs5)
...
func TestMesh_persistLayerHashes(t *testing.T) {
	...
	l2 := addLayer(r, types.GetEffectiveGenesis()+1, 5, msh)
...
func addManyTXsToPool(r *require.Assertions, msh *Mesh, layerID types.LayerID, numOfTxs int) ([]types.TransactionID, []*types.Transaction) {
...
func TestMesh_AddBlockWithTxs(t *testing.T) {
	...
	numTXs := 6
	txIDs, _ := addManyTXsToPool(r, mesh, 1, numTXs)

	block := types.NewExistingBlock(1, []byte("data"), txIDs)
