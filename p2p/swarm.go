...
func newSwarm(ctx context.Context, config config.Config, logger log.Log, datadir string) (*swarm, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create a p2p identity, err: %v", err)
	}
	...
	s.discover = discovery.New(l, config.SwarmConfig, s.udpServer, s.logger) // create table and discovery protocol
